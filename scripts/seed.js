...
async function main() {
  ...
  // Funder sends ether to the DAO treasury
  ...
  for (var i = 0; i < 3; i++) {
    transaction = await dao
      .connect(investor1)
      .createProposal(`Proposal ${i + 1}`, `Proposal Description ${i + 1}`, tokens(100), recipient.address);
    await transaction.wait();
  ...
  transaction = await dao
    .connect(investor1)
    .createProposal(`Proposal 4`, "One More Proposal", tokens(100), recipient.address);
  await transaction.wait();
